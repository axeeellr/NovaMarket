...
import { faArrowLeft, faArrowRight, faCircle, faUpDownLeftRight } from '@fortawesome/free-solid-svg-icons';

const fruits = 'https://novamarket-img.s3.us-east-2.amazonaws.com/dairy.jpg';
...
const App = () => {
    ...
    const toggleMenuProductsVisibility = (point, e) => {
        ...
        if (point.id === selectedPoint?.id) {
            ...
            if (point) {
                axios.get(`https://novamarketbackend.onrender.com/api/products/${point.id}`)
                    .then(response => {
                ...
                {arrows
                    ...
                    .map(arrow => (
                        <div
                            key={arrow.id}
                            id={`arrow-${arrow.id}`}
                            className={arrow.id === 'Atrás' ? 'arrow arrowAtras arrowCarnesAtras' : 'arrow arrowAdelante arrowLimpieza'}
                            style={{
                                left: `${arrow.x * 100}%`,
                                top: `${arrow.y * 100}%`,
                            }}
